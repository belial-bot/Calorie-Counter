...
Auf Android nimmt der Scanner die im Browser eingebaute Erkennung. Wo es die nicht
gibt — auf dem iPhone zum Beispiel — liest `js/ean.js` selbst: entlang schräg
gelegter Schnitte durch das Bild, in mehreren Winkeln über aufeinanderfolgende
Bilder verteilt. Das ist der Grund, warum auch ein schief gehaltener oder
zerknitterter Code gelesen wird.

Was dabei hilft:

- **Näher heran.** Nicht die Bildgröße entscheidet, sondern wie viele Bildpunkte
  auf einen Strich fallen. Ab etwa vier übersteht der Code auch leichte Unschärfe,
  bei zwei ist er beim kleinsten Wackler weg.
- **Licht.** Kein Gegenlicht, kein Schatten der eigenen Hand quer über dem Code.
- **Schräg halten ist erlaubt**, hochkant auch. Nur direkt von der Seite auf eine
  stark gewölbte Dose bleibt schwierig.

Ein Code gilt erst, wenn er zweimal gleich gelesen wurde. Das kostet den Bruchteil
einer Sekunde und verhindert, dass Schrift auf der Packung als Zahlenfolge
durchgeht.
...
| Welche Produkte die Suche überhaupt liefert | `js/off.js`, `clause()` und `buildQuery()` |
| Reihenfolge der Suchtreffer | `js/search.js`, die Gewichte in `score()` |
| Winkel und Dichte beim Barcode-Scan | `js/scanner.js`, `PLAN` |
...
**Suche.** Gefragt wird `search.openfoodfacts.org`. Der Dienst versteht Lucene-Syntax,
und daran hängt die Trefferqualität: ein Wort ohne Feldnamen sucht über Name, Gattung,
Kategorie *und* Marke — bei „Milch" gewinnt so ein geriebener Mozzarella, weil das Wort
in seinen Kategorien steht. `js/off.js` stellt deshalb jedem Suchwort einen Filter auf
`product_name` und `brands` zur Seite. Erst wenn das zu wenig hergibt, wird nachgefasst:
ohne Länderfilter, dann ohne Namensfilter. Im Regelfall ist eine Suche genau eine Anfrage.
